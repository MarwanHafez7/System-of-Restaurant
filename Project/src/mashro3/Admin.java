...
public class Admin {
    ...
    MySystem s=new MySystem();
       if (s.getDelievry_staff()==0){
            db.connect();
       db.update_order("waiting");
           System.out.println("there's no delievery right now please wait have an hour");
 }
       else {
       db.connect();
    ...
    public void cancel_booking_order()
    {
        ...
        try {
            while (r.next())
            {
                ...
                {
                    db.delete_row("orders", "confirmation", "waiting");
                }
            }
        ...
        try {
            while (r.next())
            {
             System.out.print(r.getInt("id")+" "+r.getString("orderdetails")+" "+
                     r.getString("username")+" "+r.getString("confirmtion")+" "+
